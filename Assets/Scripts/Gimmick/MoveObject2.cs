        ...
        if(movePoint != null && movePoint.Length > 1 && rb != null)
         {
             ...
             if (!returnPoint)
             {
                 ...
                 if (Vector2.Distance(transform.position, movePoint[nextPoint]) > 0.1f)
                 {
                     //現在地から次のポイントへのベクトルを作成
                     Vector2 toVector = Vector2.MoveTowards(transform.position, movePoint[nextPoint], speed * Time.deltaTime);
